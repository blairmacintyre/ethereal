import { ClampToEdgeWrapping, CompressedTexture, DoubleSide, LinearFilter, LinearMipMapLinearFilter, Matrix4, Mesh, MeshBasicMaterial, MeshDepthMaterial, Object3D, PlaneGeometry, RGBADepthPacking, Vector3, VideoTexture, Texture, TextureLoader, CanvasTexture } from "three";
...
export class WebLayer3D extends Object3D {
  ...
  get texture() {
    ...
    if (_layer.isMediaElement) {
      ...
      if (!t || t.image && (media as any).src !== t.image.src) {
        if (t) t.dispose()
        t = _layer.isVideoElement ? new VideoTexture(media as HTMLVideoElement) :
          _layer.isCanvasElement ? new CanvasTexture(media as HTMLCanvasElement) : 
            new TextureLoader().load( (media as HTMLImageElement).src )
        t.wrapS = ClampToEdgeWrapping
        t.wrapT = ClampToEdgeWrapping
        t.minFilter = _layer.isVideoElement ? LinearFilter : LinearMipMapLinearFilter
  ...
  querySelectorAll(selector: string) : WebLayer3D[] {
    ...
    return Array.from(elements).map<WebLayer3D>((e) => this.container.manager.layersByElement.get(e)! ).filter(l => l)
  }

  traverseLayerAncestors(
    each: (layer: WebLayer3D) => void
  ) {
  ...
  traverseLayersPreOrder(
    each: (layer: WebLayer3D) => boolean|void
  ) {
  ...
  private _updateDOMLayout() {
    ...
    this.domSize.set(
      ...
      Math.max(pixelSize * fullHeight, 10e-6),
      1
    )
